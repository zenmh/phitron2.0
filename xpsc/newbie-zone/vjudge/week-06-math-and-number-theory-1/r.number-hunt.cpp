...
void run()
{
  ...
  pair<ll, ll> primes, primes_found = {0, 0};
  ...
  while (1)
  {
    if (prime(n) && !primes_found.first)
    {
      primes.first = n;
      primes_found.first = 1;
    }
    else if (prime(n) && !primes_found.second)
    {
      primes.second = n;
      primes_found.second = 1;
      break;
    }
  ...
  cout << primes.first * primes.second << nl;
}
